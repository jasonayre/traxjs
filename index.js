...
var apply = __webpack_require__(26),
    ...
    baseInvoke = __webpack_require__(86),
...
class topic_Subscription {
  ...
  static matchesConditions(message, conditions) {
    return true;
  }
  ...
  constructor({
    ...
    assign_default()(this, {
      ...
      is_one_time,
      observable,
      _onBeforeDisposeCallbacks: onBeforeDispose
    });
  }

  get has_any_conditions() {
    return !!(this.$if || this.$unless);
  }

  passesConditions(message) {
    if (!this.has_any_conditions) {
      return true;
    }
  ...
  _passesIfConditions(message) {
    return isFunction_default()(this.$if) ? this.$if.bind(this.scope || this)(message) : this.constructor.matchesConditions(message, this.$if);
  }

  _passesUnlessConditions(message) {
    return !(isFunction_default()(this.$unless) ? this.$unless.bind(this.scope || this)(message) : this.constructor.matchesConditions(message, this.$unless));
  }
  ...
  dispose() {
    if (some_default()(this._onBeforeDisposeCallbacks)) {
      invokeMap_default()(this._onBeforeDisposeCallbacks, 'call');
    }
...
class topic_Topic {
  ...
  subscribe(params = {}) {
    let subscription = isFunction_default()(params) ? new topic_Subscription({
      perform: params,
      observable: this
    }) : new topic_Subscription({
      observable: this,
      ...params
    });
  ...
  clear() {
    ...
    invokeMap_default()(subscriptions, 'dispose');
  }
...
var includes = __webpack_require__(58);
var includes_default = /*#__PURE__*/__webpack_require__.n(includes);

// EXTERNAL MODULE: ./node_modules/lodash/pick.js
var pick = __webpack_require__(59);
...
function evaluatePropertyOn(path, binding, ...rest) {
  let value = get_default()(binding, path);

  if (isFunction_default()(value)) {
    let bound = value.bind(binding);
    return some_default()(rest) ? bound(...rest) : bound();
  } else {
    return value;
  }
}
function fetch(object, path, resolver) {
  let result = get_default()(object, path);
  if (result != null) return result;
  set_default()(object, path, resolver());
  return get_default()(object, path);
}
function isRequired(param = 'param') {
...
function setProperty(obj, path, value) {
  ...
  let target = get_default()(obj, target_path);

  if (!target && target !== 0) {
    set_default()(obj, path, value);
    return get_default()(obj, path);
...
const getInstanceMethods = (obj, excluded = []) => {
  let keys = [];
  let topObject = obj;

  const onlyOriginalMethods = (p, i, arr) => {
    return !_.includes(excluded, p) && !isGetter(topObject, p) && typeof topObject[p] === 'function' && p !== 'constructor' && (i === 0 || p !== arr[i - 1]) && keys.indexOf(p) === -1;
  };

  do {
    const l = Object.getOwnPropertyNames(obj).sort().filter(onlyOriginalMethods);
    keys = keys.concat(l); // walk-up the prototype chain
...
const getInstancePropertyNames = (obj, excluded = [], exclude_private = true) => {
  let keys = [];
  let topObject = obj;

  const onlyProperties = (p, i, arr) => {
    return !includes_default()(excluded, p) && typeof topObject[p] !== 'function' && (exclude_private ? p[0] !== '_' : true) && p !== 'constructor' && (i === 0 || p !== arr[i - 1]) && keys.indexOf(p) === -1;
  };

  do {
    const l = Object.getOwnPropertyNames(obj).sort().filter(onlyProperties);
    keys = keys.concat(l); // walk-up the prototype chain
...
function getInstanceProperties(instance) {
  ...
  return pick_default()(instance, property_names); // return reduce(property_names, (result, prop) => {
  //   result = instance[prop]
  // })
  // return map(property_names, (prop) => { return instance[prop] })
}
// CONCATENATED MODULE: ./src/mixins/has_subscriptions.js
...
const HasSubscriptions = superclass => class extends superclass {
  constructor(...args) {
    super(...args);
  ...
  $$on(_event, callback) {
    ...
    let subscription = get_default()(this, key);

    if (subscription) {
      return subscription.subscribe(callback);
    } else {
      setProperty(this, key, new topic_Topic());
      return get_default()(this, key).subscribe(callback);
    }
  }
  ...
  async $$broadcastAsync(_event, message) {
    this.$$notify(_event, message);
  }

  $$notify(_event, message) {
    let subscription = get_default()(this, `$$topic_subscriptions.${_event}`);
  ...
  $$disposeSubscriptionsForSubscribers() {
    invokeMap_default()(values_default()(this.$$topic_subscriptions), 'dispose');
  }

};
// CONCATENATED MODULE: ./src/mixwith.js
...
const Cached = mixin => wrap(mixin, superclass => {
  ...
  let application = mixin(superclass); // Cache the mixin application on the superclass
...
const BareMixin = mixin => wrap(mixin, superclass => {
  // Apply the mixin
  let application = mixin(superclass); // Attach a reference from mixin applition to wrapped mixin for RTTI
